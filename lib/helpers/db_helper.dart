...
//   Future<List<Carpets>?> getItemList({required laundryId}) async {
//     var dbClient = await db;
//     final List<Map<String, Object?>>? queryResult = await dbClient
//         ?.query('items', where: 'laundryId=?', whereArgs: [laundryId]);
//     if (queryResult != null) {
//       return (queryResult).map((e) => Carpets.fromJson(e.toString())).toList();
//     } else {
//       return [Carpets()];
//     }
//   }
...
//   Future<int> updateQuantity(Carpets item) async {
//     var dbClient = await db;
//     return await dbClient!
//         .update('items', item.toJson() as Map<String, Object?>, where: 'id = ?', whereArgs: [item.id]);
//   }
